...
export const RESUME_DATA: ResumeData = {
    ...
    "experience": [
        {
            ...
            "projects": [
                ...
                },
                {
                    ...
                    "description": "这是一个基于Streamlit开发的数据处理和转换应用，主要用于将Excel文件（OPPs）中的数据转换并导入到Access数据库（accdb）中，同时支持多个Access数据库文件的合并功能。应用支持从REVISION_12.2到REVISION_17.05等多个版本的数据处理，用户可通过直观的GUI界面选择包含Excel文件的文件夹，系统自动检测文件数量并估算处理时间，支持自定义输出数据库文件名和保存路径，带有实时进度条和状态显示。应用还具备数据库合并功能，用户可选择包含多个ACCDB文件的源文件夹，系统自动将所有ACCDB文件的数据合并到目标文件。应用特点包括详细的日志记录、性能监控、设置持久化、版本一致性检查以及自动错误和警告检测，适合需要批量处理大量Excel数据转换为Access数据库，或合并多个Access数据库的场景，能有效提高工作效率，减少人工操作和错误率。"
                },
                {
                    "name": "西班牙团队技术支持与交流",
    ...
    "personal_projects": [
      {
        "name": "量化投资分析系统",
        "description": "这是一个用于学习量化交易的Python项目，主要实现了股票交易策略的回测和分析功能。核心功能包括从AKShare等数据源获取股票历史数据和指数成分股信息，使用移动平均线交叉策略进行多股票同时回测，并提供详细的回测结果分析，如总利润、胜率、交易次数等关键指标。项目采用模块化设计，包含数据获取与管理、策略回测、结果分析和可视化等核心模块，支持配置初始资金、交易成本、移动平均线类型等参数，通过main.py直接运行或test.py进行多次回测比较，适合学习量化交易基础知识和策略验证。"
      },
      {
        "name": "图片水印清除软件",
        "description": "这是一款基于Python和Streamlit构建的图片水印清除器应用，专注于高效批量处理图片水印，特别优化了对'豆包AI生成'等右下角水印的自动清除。应用提供直观的Web界面，支持自定义输入输出目录、批量图片处理以及实时的原始与处理后图片对比功能，采用OpenCV的inpaint算法确保高质量修复效果。该应用操作简便，用户只需选择图片目录并点击处理按钮即可完成批量水印清除，同时具备设置持久化、中文路径支持和容错处理机制，适合各类用户快速处理带有统一水印的图片，显著提高工作效率。"
      },
      {
        "name": "道路工程路书自动化处理软件",
        "description": "这是一个基于Streamlit的公路图表生成应用，用于公路工程设计与规划，可从Excel输入表读取赔偿树木青苗、公路用地、砍树挖根、拆迁电力电讯设施、拆迁建筑物及拆除安全设施等多类型工程数据，支持设置多段桩号范围生成公路图表，提供图标分类管理、预览、压缩与自动添加功能，并支持配置保存路径、工作表距离、单元格尺寸等生成参数，通过选择保存文件夹、填写Excel输入表、设置桩号、生成路表数据、查看修改、添加图标等步骤，快速输出包含完整数据和图标的Excel公路图表，帮助设计人员提高工作效率。"
      }
    ]
};
